...
if (isset($_GET['ajax'])) {
    ...
    try {
        ...
        if ($_GET['ajax'] === 'mark_all_read') {
            ...
            $pdo->exec("UPDATE public.incident_report SET is_read = true WHERE LOWER(COALESCE(category,''))='seedling' AND is_read=false");
            $pdo->commit();
            echo json_encode(['ok' => true]);
            exit;
        }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    ...
    try {
        ...
        if ($action === 'mark_all_read') {
            // notifications addressed to 'seedling'
            $pdo->exec("UPDATE public.notifications SET is_read = true WHERE LOWER(COALESCE(\"to\", '')) = 'seedling'");
            $pdo->exec("UPDATE public.incident_report SET is_read = true WHERE LOWER(COALESCE(category,''))='seedling'");
            echo json_encode(['ok' => true]);
            exit();
        }
...
try {
    $notifRows = $pdo->query("SELECT n.notif_id, n.message, n.is_read, n.created_at, n.\"from\" AS notif_from, n.\"to\" AS notif_to,
        ...
        LIMIT 500");
    $seedlingNotifs = $notifRows ? $notifRows->fetchAll(PDO::FETCH_ASSOC) : [];

    $unreadPermits = (int)$pdo->query("SELECT COUNT(*) FROM public.notifications n WHERE LOWER(COALESCE(n.\"to\", ''))='seedling' AND n.is_read=false")->fetchColumn();
    $unreadSeedling = $unreadPermits;
} catch (Throwable $e) {
...
foreach ($seedlingNotifs as $nf) {
    ...
    $link = $hasIncident
        ...
        : ($hasReqpro
            ? 'seedlingsprofile.php?reqpro_id=' . urlencode((string)$nf['reqpro_id'])
            : ($hasApproval
                ? 'user_requestseedlings.php?approval_id=' . urlencode((string)$nf['approval_id'])
                : 'seedlingsnotification.php'));
...
<head>
    ...
    <title>Seedlings | Notifications</title>
...
<body>
    ...
    <header>
        ...
        <div class="nav-container">
                    ...
                    <div class="notification-list" id="seedlingNotifList">
                        ...
                        if (empty($combined)): ?>
                            ...
                            foreach ($combined as $item):
                                ...
                                $notifTitle = !empty($item['raw']['incident_id']) ? 'Incident report' : (!empty($item['raw']['reqpro_id']) ? 'Profile update' : 'Seedling Request');
                            ?>
            ...
            <!-- Profile Dropdown -->
            <div class="nav-item dropdown">
                <div class="nav-icon <?php echo $current_page === 'seedlingsprofile' ? 'active' : ''; ?>">
    ...
    <div class="notifications-container">
        ...
        <div class="notification-tabs">
            ...
            <div id="unread-tab" class="tab">Unread <span class="tab-badge"><?= (int)$unreadSeedling ?></span></div>
        </div>
    ...
    <div class="modal" id="notification-modal">
        <div class="modal-content">
            ...
            <div class="modal-header">
                <h2>Seedling Notification Details</h2>
            </div>
            <div class="modal-body">
                <p><strong>Category:</strong> Seedling Notification</p>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            ...
            // initialize badges
            setNavBadge(<?= (int)$unreadSeedling ?>);
            setTabBadge(<?= (int)$unreadSeedling ?>);
        });
    </script>
</body>
