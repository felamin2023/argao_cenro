...
/**
 * treepermit.php (Tree Cutting Admin UI)
 * - Lists approvals with request_type IN ('treecut','lumber','wood','chainsaw')
 ...
 * - ADMIN BELL UI: shows only Tree Cutting notifications ("to" = 'Tree Cutting') and incidents (category = 'Tree Cutting')
 * - View modal opens instantly with skeleton loading state
 * - “Mark all as read” works for:
 *        • public.notifications where "to" ILIKE 'Tree Cutting'
...
try {
    ...
    // allow either "forestry" or explicit "tree cutting"
...
if (isset($_GET['ajax']) && $_GET['ajax'] === 'decide') {
    ...
    try {
        ...
        if ($action === 'approve') {
            ...
            ")->execute([
                ':aid'      => $approvalId,
                ':msg'      => 'Your tree-cutting related request was approved.',
                ':fromDept' => $fromDept,
                ':toUser'   => $toUserId
            ]);
            ...
            ")->execute([
                ':aid'      => $approvalId,
                ':msg'      => 'Your tree-cutting related request was rejected. Reason: ' . $reason,
                ':fromDept' => $fromDept,
                ':toUser'   => $toUserId
            ]);
...
if (isset($_GET['ajax']) && $_GET['ajax'] === 'mark_notifs_for_approval') {
    ...
    try {
        ...
        error_log('[TREE-MARK-READ] ' . $e->getMessage());
        echo json_encode(['ok' => false, 'error' => 'server error']);
    }
    exit();
}
            ...
            document.getElementById('statusTableBody')?.addEventListener('click', async (e) => {
                ...
                try {
                    ...
                    if (res && res.ok) {
                        ...
                        // also mark in the open dropdown, if present
