...
try {
    ...
    error_log('[TREECUT CLIENTS] ' . $e->getMessage());

    $clientRows = [];
}

function format_treecut_client_option(array $client): string

{

    $value = htmlspecialchars((string)($client['client_id'] ?? ''), ENT_QUOTES);

    $first = htmlspecialchars((string)($client['first_name'] ?? ''), ENT_QUOTES);

    $middle = htmlspecialchars((string)($client['middle_name'] ?? ''), ENT_QUOTES);

    $last = htmlspecialchars((string)($client['last_name'] ?? ''), ENT_QUOTES);

    $street = htmlspecialchars((string)($client['sitio_street'] ?? ''), ENT_QUOTES);

    $barangay = htmlspecialchars((string)($client['barangay'] ?? ''), ENT_QUOTES);

    $municipality = htmlspecialchars((string)($client['municipality'] ?? ''), ENT_QUOTES);

    $city = htmlspecialchars((string)($client['city'] ?? ''), ENT_QUOTES);

    $contact = htmlspecialchars((string)($client['contact_number'] ?? ''), ENT_QUOTES);



    $nameParts = array_filter([$first, $middle, $last], function ($v) {
    ...
    $label = $displayName !== '' ? $displayName : 'Unnamed client';
    ...
    if ($barangay !== '') $addrParts[] = 'Brgy. ' . $barangay;

    if ($municipality !== '') $addrParts[] = $municipality;

    if ($city !== '') $addrParts[] = $city;

    $addrLabel = implode(', ', $addrParts);
    ...
    $dataset = sprintf(
        ...
        'data-first="%s" data-middle="%s" data-last="%s" data-street="%s" data-barangay="%s" data-municipality="%s" data-city="%s" data-contact="%s"',
    ...
    return "<option value=\"{$value}\" {$dataset}>{$label}</option>";
}
...
<body>
        ...
        <div class="requirements-form">
            ...
            <div class="form-body">
                ...
                <div style="display:flex;gap:8px;align-items:center;margin-bottom:10px;flex-wrap:wrap;">
                    ...
                    <small style="opacity:.8;margin-left:6px">Choose <b>Existing client</b> to reuse records already in the system.</small>
                ...
                <div id="existingClientRow" class="name-fields" style="display:none;">
                    ...
                    <div class="name-field" style="grid-column:1/-1;">
                        ...
                        <select id="clientPick" class="input" style="height:40px;">
                        ...
                        <!-- <div id="clientDetails" class="client-details" hidden>
                            <strong>Address</strong>: <span id="clientDetailsAddress">—</span><br>
                            <strong>Contact No.</strong>: <span id="clientDetailsContact">—</span>
                        </div> -->
                ...
                <div id="newClientRow" class="name-fields">
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const clientPickEl = document.getElementById('clientPick');
            ...
            const clientDetailsAddressEl = document.getElementById('clientDetailsAddress');

            const clientDetailsContactEl = document.getElementById('clientDetailsContact');

            const nameFields = [firstNameEl, middleNameEl, lastNameEl];



            function updateClientDetails(dataset) {
                if (!clientDetailsEl || !clientDetailsAddressEl || !clientDetailsContactEl) return;
                const addrParts = [];
                if (dataset.street) addrParts.push(dataset.street);
                if (dataset.barangay) addrParts.push(`Brgy. ${dataset.barangay}`);
                if (dataset.municipality) addrParts.push(dataset.municipality);
                const addressText = addrParts.length ? addrParts.join(', ') : '—';
                clientDetailsAddressEl.textContent = addressText;
                clientDetailsContactEl.textContent = dataset.contact || '—';
                clientDetailsEl.hidden = !(addrParts.length || dataset.contact);
            }

            function applySelectedClient() {

                if (!clientPickEl) return;

                const dataset = (clientPickEl.options[clientPickEl.selectedIndex] ?? {}).dataset ?? {};

                updateClientDetails(dataset);
                ...
                assign(firstNameEl, dataset.first);

                assign(middleNameEl, dataset.middle);

                assign(lastNameEl, dataset.last);

                assign(streetEl, dataset.street);

                assign(barangayEl, dataset.barangay);

                assign(municipalityEl, dataset.municipality);

                assign(contactEl, dataset.contact);
            ...
            function setClientMode(mode) {
                ...
                if (clientModeEl) clientModeEl.value = isExisting ? 'existing' : 'new';

                if (existingClientRow) existingClientRow.style.display = isExisting ? 'flex' : 'none';

                if (newClientRow) newClientRow.style.display = isExisting ? 'none' : 'flex';

                if (btnExisting) btnExisting.style.display = isExisting ? 'none' : 'flex';
                ...
                if (isExisting) {
                    ...
                    if (clientPickEl) clientPickEl.value = '';
            ...
            clientPickEl?.addEventListener('change', () => {

                if ((clientModeEl?.value ?? '') === 'existing') {
            ...
            if (submitApplicationBtn) {
                submitApplicationBtn.addEventListener('click', function(e) {
                    ...
                    // Optional: use your text-field validation if available
                    ...
                    requiredFiles.forEach(id => {
                        ...
                        if (!file) {
                            ...
                            // Clear error styling if it was previously red
                            if (nameEl.textContent === 'Please attach file') {
                                nameEl.textContent = file.name || 'Please attach file';
                            }
                            nameEl.style.color = '';
                            nameEl.style.fontWeight = '';
                        }
