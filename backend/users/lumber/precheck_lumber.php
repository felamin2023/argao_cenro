...
try {
    ...
    // ---------- If no client found ----------
    // Special case: attempting a RENEWAL but no client on record -> must have an approved NEW first.
    if (!$client_id) {
        if ($desired === 'renewal') {
    ...
    // ---------- Gather status flags (for LUMBER) ----------
    $hasPendingNew      = false;
    $hasApprovedNew     = false;
    $hasPendingRenewal  = false;

    // any PENDING NEW lumber?
    $stmt = $pdo->prepare("
  ...
  ");
    ...
    If user is submitting NEW (or didn't specify):
      ...
      - block if there's a pending RENEWAL (avoid parallel flows)
  ...
  */
    ...
    if ($hasApprovedNew) {
        // No pending renewal at this point -> suggest moving to renewal
