...
try {
    ...
    $emit_existing = function (string $cid, string $desired) use ($pdo) {
        ...
        // unexpired check (any released lumber permit with expiry >= today)
        $uq = $pdo->prepare("
            SELECT 1
        ...
        if ($desired === 'new') {
            ...
            if ($has_pending_new) {
                $block = 'pending_new';
                $message = 'You already have a pending NEW lumber application.';
            } elseif ($has_pending_renewal) {
                $block = 'pending_renewal';
                $message = 'You already have a pending lumber application. Please wait for the update first.';
            } elseif ($has_unexpired) {
                $block = 'unexpired_permit';
                $message = 'You still have an unexpired lumber permit. You cannot file a new application.';
            }
            ...
            if ($has_pending_new || $has_pending_renewal) {
                ...
                $block = 'unexpired_permit';
                $message = 'You still have an unexpired lumber permit. Please wait until it expires to renew.';
            }
        }
        ...
        echo json_encode([
            ...
            'client'   => [
                ...
                'first_name'  => (string)($nrow['first_name']  ?? ''),
                'middle_name' => (string)($nrow['middle_name'] ?? ''),
                'last_name'   => (string)($nrow['last_name']   ?? '')
            ],
            'flags'    => [
