...
// blended similarity (levenshtein + prefix + metaphone bonus)
...
try {
  ...
  // Optional: explicit client to use (from "Use existing" click)
  $override_client_id = trim((string)($_POST['use_client_id'] ?? $_POST['use_existing_client_id'] ?? ''));

  // If nothing useful was typed, just let the UI proceed (client will be created later)
  ...
  // 1) If the UI sent an explicit existing id, verify it exists and use it
  ...
  // 2) Else try exact (case-insensitive) name match
  if (!$client_id) {
    $nf = norm($first);
    $nm = norm($middle);
    $nl = norm($last);
  ...
  // 3) If still no client and no override, fuzzy unordered suggestion (NO BLOCKS HERE)
  if (!$client_id && $override_client_id === '') {
    ...
    // broad candidate pool (any 2-letter prefix match on any part)
    $q = $pdo->prepare("
    ...
    if ($best && $bestScore >= $THRESHOLD) {
      ...
      // Return ONLY the candidate; DO NOT compute or return any blocks yet.
      echo json_encode([
        'ok' => true,
        'needs_confirm' => true,
        'candidates' => [[
        ...
        // legacy convenience keys for current UI
    ...
    // No strong suggestion -> proceed as brand new; UI will create a client later.
    echo json_encode(['ok' => true]);
    exit;
  }

  // If we resolved a client (override / exact), compute status flags + return name echo for UI
  if ($client_id) {
    // include name echo
