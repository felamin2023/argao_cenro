...
$check_stmt = $conn->prepare('SELECT COUNT(*) FROM profile_update_requests WHERE email = ? AND status = "pending"');
...
if ($count > 0) {
    echo json_encode(['success' => false, 'error' => "You've already requested a profile update."]);
    $conn->close();
    exit();
}
